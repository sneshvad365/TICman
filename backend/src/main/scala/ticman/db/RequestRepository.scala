...
class PostgresRequestRepository(dataSource: DataSource) extends RequestRepository:
  ...
  override def findByCollectionId(collectionId: UUID): Seq[HttpRequest] =
    ...
    try
      val stmt = conn.prepareStatement(
        "SELECT id, collection_id, name, method, url_template, headers::text, body FROM requests WHERE collection_id = ? ORDER BY created_at ASC"
      )
  ...
  override def findById(id: UUID): Option[HttpRequest] =
    ...
    try
      val stmt = conn.prepareStatement(
        "SELECT id, collection_id, name, method, url_template, headers::text, body FROM requests WHERE id = ?"
      )
      stmt.setObject(1, id)
      val rs     = stmt.executeQuery()
  ...
  override def create(collectionId: UUID, req: SaveRequestRequest): HttpRequest =
    ...
    try
      val stmt = conn.prepareStatement(
        "INSERT INTO requests (collection_id, name, method, url_template, headers, body) VALUES (?, ?, ?, ?, ?::jsonb, ?) RETURNING id"
      )
      stmt.setObject(1, collectionId)
      stmt.setString(2, req.name)
      stmt.setString(3, req.method)
      stmt.setString(4, req.urlTemplate)
      stmt.setString(5, write(req.headers))
      stmt.setString(6, req.body.orNull)
      val rs = stmt.executeQuery()
      rs.next()
  ...
  override def update(id: UUID, req: SaveRequestRequest): HttpRequest =
    ...
    try
      val stmt = conn.prepareStatement(
        "UPDATE requests SET name=?, method=?, url_template=?, headers=?::jsonb, body=?, updated_at=now() WHERE id=? RETURNING collection_id"
      )
